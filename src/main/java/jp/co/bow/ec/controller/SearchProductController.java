...
@Controller
@SessionAttributes({"size","color","product","products","userEntity","count","searchProductInfoModel","productEntity"})
public class SearchProductController {
	...
	public ProductEntity setUpProductEntity() {
	...
	public String getSearchResult(@ModelAttribute SearchProductInfoModel searchProductInfoModel, Model model, RedirectAttributes attributes) {
		...
		//検索結果を取得
		List<ProductEntity> pros = searchProductService.searchAllResult(entity);
		//最大値に検索結果のListの数を代入
		max = pros.size();
	...
	/*ページング機能
	　商品検索結果、次へボタン押下*/
	@RequestMapping(value="/search1",method=RequestMethod.POST,params="next")
	...
	/*ページング機能
	　商品検索結果、戻るボタン押下*/
	@RequestMapping(value="/search1",method=RequestMethod.POST,params="back")
	...
	public String toProductInfo(@ModelAttribute SearchProductInfoModel searchProductInfoModel, Model model) {
		...
		//類似商品
		List<ProductEntity> getlikeproduct = searchProductService.getLikeProduct(searchProductInfoModel.getProduct_id());
		model.addAttribute("getlikeproduct",getlikeproduct);
	...
	public String addCart(@Validated @ModelAttribute ProductInfoModel productInfoModel, BindingResult result,Model model,UserEntity userEntity,ProductEntity productEntity) {
		//ログインしている場合
		if(result.hasErrors()) {
			return "productInfo";
		}
		if(userEntity.getUser_id() != null) {
			...
			if(productEntity.getStock_quantity() < cart.getQuantity()) {
				model.addAttribute("qMessage", "在庫の上限こえとるやないか");
			}else {
				//カートに追加するメソッドを呼び出す
				searchProductService.insertCart(cart);
				model.addAttribute("cMessage", "カートに商品が追加されました");
				List<Product_CartEntity> carts = searchProductService.findCartAll(userEntity.getUser_id());
				cartCount = carts.size();
				userEntity.setCartCount(cartCount);
			}
			return "productInfo";
			//ログインしていない場合
		}else {
